...
class MileageProvider with ChangeNotifier {
  ...
  void addMileage(double distance, DateTime date, String description, double ratePerKm) {
    _mileages.add({'id': DateTime.now().toString(), 'distance': distance, 'date': date, 'description': description, 'ratePerKm': ratePerKm, 'totalCost': distance * ratePerKm});
    notifyListeners();
  }
  ...
  void updateMileage(String id, double distance, DateTime date, String description, double ratePerKm) {
    ...
    if (index != -1) {
      _mileages[index] = {'id': id, 'distance': distance, 'date': date, 'description': description, 'ratePerKm': ratePerKm, 'totalCost': distance * ratePerKm};
      notifyListeners();
    }
  }
